...
import {
  getExportPromptPreference,
  parseExportIntent,
  setExportPromptPreference,
...
export const Home: React.FC = () => {
  ...
  const runExport = (format: ExportFormat, result: QueryResult = queryResult!) => {
    if (!selectedDatabase || !result || result.rows.length === 0) {
    ...
    const doExport = () => {
      ...
      message.success(
        `Exported ${result.rowCount} rows to ${format.toUpperCase()} (${filename})`
      );
      setShowExportPrompt(false);
    };

    if (shouldWarnLarge(result.rows.length)) {
      Modal.confirm({
  ...
  const handleExportJSON = () => runExport("json");

  const maybeAskExport = (result: QueryResult) => {
    if (result.rowCount > 0 && getExportPromptPreference() === "ask") {
  ...
  const executeSqlAndMaybeExport = async (
    sqlText: string,
    ...
    try {
      ...
      if (exportFormat && response.data.rowCount > 0) {
        runExport(exportFormat, response.data);
      } else {
        maybeAskExport(response.data);
      }
    } catch (error: any) {
  ...
  const promptForFormat = (): Promise<ExportFormat | null> =>
    new Promise((resolve) => {
      Modal.confirm({
        title: "选择导出格式",
        icon: <ExclamationCircleOutlined />,
        content: "请选择要将结果导出为 CSV 还是 JSON。",
        okText: "CSV",
        cancelText: "JSON",
        onOk: () => resolve("csv"),
        onCancel: () => resolve("json"),
      });
    });
  ...
  const handleGenerateSQL = async (prompt: string) => {
    ...
    const parsed = parseExportIntent(prompt);

    // Export current result only
    if (parsed.intent === "export") {
      if (!queryResult || queryResult.rows.length === 0) {
        message.warning("当前没有可导出的查询结果，请先执行查询");
        return;
      }
      const format = parsed.format ?? (await promptForFormat());
    ...
    try {
      const response = await apiClient.post<{ sql: string; explanation: string }>(
        ...
        { prompt: parsed.cleanedPrompt || prompt }
      );
      setSql(response.data.sql);
      setActiveTab("manual");

      if (parsed.intent === "query_and_export") {
        message.success("已生成 SQL，正在执行并导出…");
        const format = parsed.format ?? (await promptForFormat()) ?? "csv";
        await executeSqlAndMaybeExport(response.data.sql, format);
      } else {
        // Natural language query: generate → execute → AI asks about export
        message.success("已生成 SQL，正在执行查询…");
        await executeSqlAndMaybeExport(response.data.sql, null);
      }
    } catch (error: any) {
          ...
          >
            {showExportPrompt && (
              <ExportPrompt
                ...
                onNeverAsk={() => {
                  setExportPromptPreference("never");
                  setShowExportPrompt(false);
                  message.info("已关闭导出询问，可随时使用 EXPORT 按钮");
                }}
              />
            )}
            <Table
