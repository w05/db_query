...
class DatabaseAdapterRegistry:
    ...
    def create_adapter(
        ...
        Useful for one-off operations like connection tests where the URL
        must not be reused from a previous attempt.
        """
    ...
    def get_adapter(
        ...
        if existing is not None:
            # If URL changed (e.g. user updated credentials), replace cached adapter
            if existing.config.url != config.url:
                logger.info(
                    f"Replacing cached adapter for {config.name} due to URL change"
                )
