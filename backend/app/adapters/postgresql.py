...
class PostgreSQLAdapter(DatabaseAdapter):
    ...
    async def test_connection(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            # Prefer explicit DSN parse; asyncpg URL auth failures often surface
            # as a vague "connection was closed in the middle of operation".
            conn = await asyncpg.connect(dsn=self.config.url)
            await conn.close()
            return True, None
        except asyncpg.InvalidPasswordError as e:
            return False, f"Invalid password: {e}"
        except asyncpg.InvalidAuthorizationSpecificationError as e:
            return False, f"Authorization failed (check username/database): {e}"
        except asyncpg.exceptions.ConnectionDoesNotExistError:
            return (
                False,
                "Connection closed during handshake. Usually wrong username/password "
                "or user does not exist. Use PostgreSQL user 'postgres' (not Windows "
                "login), e.g. postgresql://postgres:PASSWORD@127.0.0.1:5432/postgres",
            )
